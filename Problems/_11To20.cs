    ...
    public partial class _11To20
    {
        ...
        public void Problem12()
        {
            if (false)
            {
                ...
                Func<BigInteger, BigInteger[]> divisorCount
                    = (i) => {
                        if (i % 2 == 0)
                        {
                            var nPlus1Divs = Sequences.UniqueDivisors(i + 1).Count;
                            var nDivs = Sequences.UniqueDivisors(i / 2).Count;
                        ...
                        {
                            var nPlus1Divs = Sequences.UniqueDivisors((i + 1) / 2).Count;
                            var nDivs = Sequences.UniqueDivisors(i).Count;
                            return new BigInteger[] {
                                nPlus1Divs,
                                nDivs,
                                nPlus1Divs + nDivs,
                ...
                BigInteger n = 4;
                BigInteger highestCount = 0;
                while (highestCount < 500)
                {
                    var divisorCounts = divisorCount(n);
                    Console.WriteLine(string.Join(" ", divisorCounts));
                    highestCount = divisorCounts[2] < highestCount
                        ? highestCount
                        : divisorCounts[2];
            ...
            var primes = Sequences.PrimesUnder(1_000_000)
                .ToList();

            Func<BigInteger, int, BigInteger> p = (take, power) =>
            {
                return
                    primes
                        .Take((int)take)
                        .Select(x => BigInteger.Pow(x, power))
                        .Aggregate(1, (BigInteger x, BigInteger y) => x * y);

                //return Sequences.UniqueDivisors(
                //    product
                //).Count;
            };
            Func<BigInteger, int, BigInteger> d = (take, power) =>
            {
                // This is a shortcut.
                return take * (power + 1);
            ...
            foreach (var take in Enumerable.Range(2, 25))
            {
                foreach (var power in Enumerable.Range(2, 25))
                {
                    var result = d(take, power);
                    Console.WriteLine($"d({take}, {power}) = {result}, {p(take, power)}");
                }
            }
            ...
            //var counter = 0;
            //var highestCount = 0;
            //foreach (var triangleNumber in Sequences.TriangleNumbers(100_000_000))
            //{


            //    var count = Sequences.UniqueDivisors(triangleNumber).Count;
            //    highestCount = Math.Max(count, highestCount);
            //    Console.WriteLine($"{triangleNumber} {count} {highestCount}");
            //    counter++;
            //    if (counter > 100_0)
            //        break;
        ...
        public void Problem15()
        {
            ...
            for (long x = 20; 0 <= x; x--)
                for (long y = 20; 0 <= y; y--)
                {
                    if (x == 20 && y == 20)
                        corners[y, x] = 1;
                    else if (x == 20) // check down
                        corners[y,x] = corners[y + 1, x];
                    else if (y == 20) // check right
                        corners[y, x] = corners[y, x + 1];
                    else // check down and right
                        corners[y, x] = corners[y + 1, x] + corners[y, x + 1];
                }
